    ...
    void NextStageSetting()
    {
        switch (playerMovement.currentStage) // 씬 이동, 플레이어 위치 설정
        {
            case 2:
                ...
                break;
            case 3:
                StartCoroutine(LoadNextStageAsync("Stage3", new Vector3(0f, 4f, 0f), Quaternion.Euler(0, 0, 0)));
                previousStage = playerMovement.currentStage;
                break;
            case 4:
                ...
                break;
            case 5:
                StartCoroutine(LoadNextStageAsync("Stage5", new Vector3(92f, 40f, -36.5f), Quaternion.Euler(0, 180, 0))); // 시작
                //StartCoroutine(LoadNextStageAsync("Stage5", new Vector3(-11f, 49f, -28f), Quaternion.Euler(0, 180, 0))); // 3층
                //StartCoroutine(LoadNextStageAsync("Stage5", new Vector3(-18f, 22.5f, 4f), Quaternion.Euler(0, 180, 0))); // 부엌
                previousStage = playerMovement.currentStage;
                break;
        }
    }
